(function () {
  ...
  const SIBLING_DISCOUNT = Object.freeze({
    1: 1.0, 2: 0.9, 3: 0.8, 4: 0.65, 5: 0.45, 6: 0.25
  });
  ...
  function calculate({ childAgeType, netIncome, siblingsCount, careHours, highestRate }) {
    if (childAgeType === 'kindergarten') {
      return {
        fee: 0, baseFee: 0, info: '', showEntlastung: false,
        meta: { rule: `Beitragsfrei (ab ${META.KIGA_AGE_THRESHOLD} Jahren)`, notes: ['Die Betreuung im Kindergartenalter ist in Brandenburg beitragsfrei.'] }
      };
    }
    ...
    if (siblingsCount >= META.SIBLINGS_FREE_THRESHOLD) {
      return {
        fee: 0, baseFee: 0, info: '', showEntlastung: false,
        meta: { rule: `Beitragsfrei (≥ ${META.SIBLINGS_FREE_THRESHOLD} Kinder)`, notes: ['Bei 7 oder mehr unterhaltsberechtigten Kindern entfällt der Elternbeitrag.'] }
      };
    }
    ...
    if (highestRate === true) {
      ...
      return { fee, baseFee: base, info, showEntlastung: false, meta: { rule: 'Höchstsatz (Satzung U3)', highestRate: true, discountFactor: factor, discountPercent: Math.max(0, Math.round((1 - factor) * 100)), notes: [] } };
    }

    if (netIncome <= LIMITS.MIN_INCOME_FREE_U3) {
      return { fee: 0, baseFee: 0, info: '', showEntlastung: true, meta: { rule: 'Beitragsfrei (Einkommen ≤ 35.000 EUR)', notes: ['Gemäß Elternbeitragsentlastungsgesetz.'] } };
    }
    ...
    if (netIncome >= LIMITS.MIN_INCOME_ENTLASTUNG_U3 && netIncome <= LIMITS.MAX_INCOME_ENTLASTUNG_U3) {
      ...
      return { fee: base, baseFee: base, info: '', showEntlastung: true, meta: { rule: 'Reduzierter Beitrag (Entlastung U3)', discountFactor: 1, notes: ['Kein zusätzlicher Geschwisterrabatt in diesem Einkommensbereich.', 'Rechtsgrundlage: Elternbeitragsentlastungsgesetz.'] } };
    }
    ...
    if (netIncome >= LIMITS.MIN_INCOME_SATZUNG_U3) {
      ...
      const meta = { rule: 'Regulärer Beitrag (Satzung U3)', discountFactor: factor, discountPercent: percent, notes: [] };
      if (siblingsCount > 1 && factor < 1) {
    ...
    return { fee: 0, baseFee: 0, info: '', showEntlastung: true, meta: { rule: 'Beitragsfrei (Einkommen U3 < 35k)', notes: [] } };
  }
  ...
  function buildExplanation(result, input) {
    ...
    const discountPercent = discountApplied ? (typeof meta.discountPercent === 'number' ? `${meta.discountPercent}%` : `${(100 - meta.discountFactor * 100).toFixed(0)}%`) : '';
    ...
    const notes = (meta?.notes || []).map(n => `<li>${n}</li>`).join('');
    ...
    const grid = rows.map(([l, v]) => `<div class="label">${l}</div><div class="value">${v}</div>`).join('');

    return `<div class="explain-title"><strong>${meta?.rule || 'Zusammensetzung'}</strong></div><div class="explain-grid">${grid}</div>${notes ? `<ul class="explain-notes">${notes}</ul>` : ''}`;
  }
